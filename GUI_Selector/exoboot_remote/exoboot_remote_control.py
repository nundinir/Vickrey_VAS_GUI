...
from constants import DETROIT_TIMEZONE, DATETIME_FORMATTER_LESS_SEC, GSE_MODE
...
class ExobootRemoteClient:
    ...
    def get_subject_info(self):
        ...
        print(subject_info)
        return (
            subject_info.startstamp,
            subject_info.subjectID,
            subject_info.trial_type,
            subject_info.trial_cond,
            subject_info.description,
            subject_info.usebackup,
        )
    ...
    def set_torques(self, peak_torque_left=0, peak_torque_right=0):
        torque_msg = pb2.torques(
            peak_torque_left=peak_torque_left, peak_torque_right=peak_torque_right
        )
    ...
    def comparison_result(self, pres, prop, T_ref, T_comp, truth, answer):
        compmsg = pb2.comparison(
            pres=pres, prop=prop, T_ref=T_ref, T_comp=T_comp, truth=truth, answer=answer
        )
...
class ExobootCommServicer(pb2_grpc.exoboot_over_networkServicer):
    ...
    def __init__(self, mainwrapper, startstamp, filingcabinet, usebackup, quit_event):
        ...
        if not loadstatus:
            if trial_type == "VICKREY":
                ...
                auctionpath = self.filingcabinet.newfile(
                    auctionname, "csv", dictkey="auction"
                )

                surveyname = "{}_{}".format(self.file_prefix, "survey")
                surveypath = self.filingcabinet.newfile(
                    surveyname, "csv", dictkey="survey"
                )
                ...
                vasresultspath = self.filingcabinet.newfile(
                    vasresultsname, "csv", dictkey="vasresults"
                )
                ...
                comparisonpath = self.filingcabinet.newfile(
                    comparisonname, "csv", dictkey="comparison"
                )
    ...
    def get_subject_info(self, nullmsg, context):
        print("GSI_SERVER: ", self.mainwrapper.condition1, self.mainwrapper.condition2)
        return pb2.subject_info(
            startstamp=self.mainwrapper.startstamp,
    ...
    def call(self, resultmsg, context):
        ...
        print(
            "Received auction results: {}, {}, {}, {}, {}".format(
                t, subject_bid, user_win_flag, current_payout, total_winnings
            )
        )
    ...
    def update_vas_info(self, vasinfomsg, context):
        ...
        overtimename = "{}_overtime_B{}_T{}_P{}".format(
            self.file_prefix, btn_num, trial, pres
        )
        overtimepath = self.filingcabinet.newfile(
            overtimename, "csv", dictkey="overtime"
        )
    ...
    def presentation_result(self, presmsg, context):
        ...
        print(
            "Received presentation results: {}, {}, {}, {}, {}".format(
                btn_option, trial, pres, torques, values
            )
        )
    ...
    def comparison_result(self, compmsg, context):
        ...
        print(
            "Received comparison results: {}, {}, {}, {}, {}, {}".format(
                pres, prop, T_ref, T_comp, truth, answer
            )
        )
    ...
    def comparison_result_stair(self, compmsg, context):
        ...
        datalist = [
            ...
            truth,
            answer,
        ]  # TODO: change data list to include stair info
...
class ExobootRemoteServerThread(BaseThread):
    ...
    def __init__(
        ...
        super().__init__(
            name=name, daemon=daemon, pause_event=pause_event, quit_event=quit_event
        )
        self.mainwrapper = mainwrapper
        self.exoboot_remote_servicer = ExobootCommServicer(
            self.mainwrapper,
            startstamp,
            filingcabinet,
            usebackup=usebackup,
