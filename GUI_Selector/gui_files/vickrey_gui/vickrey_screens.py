...
def buildpushtostartscreen(sm):
    ...
    def startbttn_CB(instance):
        ...
        if sm.statemachine.auction_tally > 0 and sm.statemachine.state:
            ...
            sm.exoboot_remote.set_torques(
                peak_torque_left=sm.peak_torque, peak_torque_right=sm.peak_torque
            )
...
def buildNumPadScreen(sm):
    ...
    bid_input = Label(
        text=decimal_format(sm.bid),
        font_size="100",
        size_hint=(4 / gn_x, 1 / gn_y),
        pos_hint={"x": 0 / gn_x, "y": 4 / gn_y},
        color=(0.5, 0.5, 0.5, 1),
    )
    ...
    btn0 = Button(
        text="0",
        font_size="70",
        size_hint=(3 / gn_x, 1 / gn_y),
        pos_hint={"x": 0 / gn_x, "y": 0 / gn_y},
    ...
    btnBCKSPC = Button(
        text="BCKSPC",
        font_size="70",
        size_hint=(1 / gn_x, 2 / gn_y),
        pos_hint={"x": 3 / gn_x, "y": 2 / gn_y},
        background_color=(0.5, 0.5, 0.5, 1),
    )
    ...
    btnCLEAR = Button(
        ...
        font_size="70",
        size_hint=(1 / gn_x, 2 / gn_y),
        pos_hint={"x": 3 / gn_x, "y": 0 / gn_y},
        background_color=(0.5, 0.5, 0.5, 1),
    )
    btnCLEAR.bind(on_press=CLEAR_CB)

    sm.cdt = CountDownTimer(
        text="",
        font_size="70",
        size_hint=(1 / gn_x, 3 / gn_y),
        pos_hint={"x": 3 / gn_x, "y": 4 / gn_y},
    )
    ...
    for i in range(1, 10):
        ...
        btn = Button(
            text="{}".format(i),
            font_size="70",
            size_hint=(1 / gn_x, 1 / gn_y),
            pos_hint={"x": x_p, "y": y_p},
            color=(1, 1, 1),
...
def buildsurveyscreen(sm):
    ...
    enjoyment_label = Label(
        text="Rate your enjoyment",
        font_size="80",
        size_hint=(1 / 3, 1 / 6),
        pos_hint={"x": 1 / 3, "y": 8.5 / 10},
        halign="center",
    )
    ...
    for i, level in enumerate(enjoyment_levels):
        btn_ = Button(
            background_normal="images/{}.png".format(level),
            size_hint=(1 / 6, 1 / 6),
            pos_hint={"x": i / 5, "y": 2 / 3},
        )
        btn_.val = i - 2
    ...
    rpe_label = Label(
        text="Rate your exertion (RPE)",
        font_size="80",
        size_hint=(1 / 3, 1 / 6),
        pos_hint={"x": 1 / 3, "y": 1 / 3},
        halign="center",
    )
    ...
    for i in range(6, 21):
        ...
        btn = Button(
            ...
            color=(1, 1, 1),
            background_normal="",
            background_color=(i_ / 14, (1 - i_ / 14), 0, 0.5),
            size_hint=(1 / 15, 1 / 5),
            pos_hint={"x": x_p, "y": y_p},
        )
        btn.val = i
